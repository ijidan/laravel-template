...
class ResponseRepo {
     ...
     */
    public function accepted(array $data = [], string $message = '', string $location = '') {
     ...
     * @return JsonResponse|JsonResource
     */
    public function created(array $data = [], string $message = '', string $location = '') {
     ...
     */
    public function fail(string $message = '', int $code = 500, array $errors = null, array $header = [], int $options = 0): JsonResponse {
        return $this->response($this->formatData(null, $message, $code, $errors), Config::get('response.error_code') ?: $code, $header, $options);
    }
     ...
     * @return JsonResponse
     */
    protected function formatPaginatedResponse(AbstractPaginator $resource, string $message = '', int $code = 200, array $headers = [], int $option = 0): JsonResponse {
     ...
     * @return mixed
     */
    protected function formatResourceCollectionResponse(JsonResource $resource, string $message = '', int $code = 200, array $headers = [], int $option = 0) {
        ...
        return tap($this->response($this->formatData($data, $message, $code), $code, $headers, $option), function ($response) use ($resource) {
     ...
     * @return mixed
     */
    protected function formatResourceResponse(JsonResource $resource, string $message = '', int $code = 200, array $headers = [], int $option = 0) {
        $resourceData = array_merge_recursive($resource->resolve(request()), $resource->with(request()), $resource->additional);

        return tap($this->response($this->formatData($resourceData, $message, $code), $code, $headers, $option), function ($response) use ($resource) {
     ...
     * @return JsonResponse
     */
    protected function response(array $data = [], int $status = 200, array $headers = [], int $options = 0): JsonResponse {
